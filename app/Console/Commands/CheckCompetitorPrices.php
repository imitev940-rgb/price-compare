...
{
    protected $signature = 'prices:check {product_id?}';
    ...
    public function handle()
    {
        ...
        if ($productId) $linksQuery->where('product_id', $productId);
        ...
        foreach ($links as $link) {
            ...
            if (empty($url) || $url === '#') {
                $link->update(['last_checked_at' => now(), 'search_status' => 'invalid_url', 'last_error' => 'Missing or placeholder URL.']);
                continue;
            }

            $storeName = strtolower(trim($link->store->name ?? ''));
            ...
            if ($storeName === 'pazaruvaj') {
                ...
                try {
                    ...
                    if ($result['success']) {
                        ...
                        $priceChanged = $oldPrice === null || round((float)$oldPrice, 2) !== round((float)$lowestPrice, 2);
                        ...
                        if ($priceChanged) {
                            ...
                            if ($ourPrice !== null && $lowestPrice !== null && (float)$lowestPrice > 0) {
                                ...
                                if ((float)$ourPrice < (float)$lowestPrice)      { $position = '#1 Best Price'; $status = 'Cheaper'; }
                                elseif ((float)$ourPrice > (float)$lowestPrice)  { $position = 'Not Best Price'; $status = 'More Expensive'; }
                                else                                              { $position = 'Same Price'; $status = 'Match'; }
                            }
                            ...
                            PriceHistory::create([
                                ...
                                'best_competitor'    => $lowestStoreName,  // реалното име на магазина
                                'position'           => $position,
                                'status'             => $status,
                                'checked_at'         => now(),
                            ]);

                            // Нотификацията показва реалния магазин от Pazaruvaj
                            $this->createPriceNotification($link, $oldPrice, $lowestPrice, $lowestStoreName . ' (чрез Pazaruvaj)');
                        }

                        $this->info('Pazaruvaj updated: lowest=' . $lowestPrice . ' €, best_store=' . $lowestStoreName . ', offers=' . ($result['offers_count'] ?? 0) . ', our position=' . ($result['our_position'] ?? '—'));
                        Log::info('Pazaruvaj offers updated', ['product_id' => $link->product_id, 'link_id' => $link->id, 'lowest_price' => $lowestPrice, 'lowest_store' => $lowestStoreName]);
                    } else {
                        $this->warn('Pazaruvaj offers not found.');
                        $link->update(['last_checked_at' => now(), 'search_status' => 'price_not_found', 'last_error' => 'Could not scrape Pazaruvaj offers.']);
                    }
                } catch (\Throwable $e) {
                    $this->error('Pazaruvaj error: ' . $e->getMessage());
                    $link->update(['last_checked_at' => now(), 'search_status' => 'error', 'last_error' => $e->getMessage()]);
                    Log::error('Pazaruvaj scrape error', ['url' => $url, 'link_id' => $link->id, 'message' => $e->getMessage()]);
                }
            ...
            try {
                ...
                if (str_contains($url, 'zora.bg')) {
                    Log::info('Zora response debug', ['url' => $url, 'status' => $statusCode, 'body_start' => mb_substr((string)($response['body'] ?? ''), 0, 300)]);
                }

                if ($statusCode < 200 || $statusCode >= 300) {
                    $this->error('Could not load page. HTTP ' . $statusCode);
                    $link->update(['last_checked_at' => now(), 'search_status' => $statusCode === 403 ? 'blocked' : 'request_failed', 'last_error' => 'HTTP status: ' . $statusCode]);
                    Log::warning('Could not load page', ['url' => $url, 'link_id' => $link->id, 'status' => $statusCode]);
                    continue;
                }
                ...
                if ($html === '') {
                    $link->update(['last_checked_at' => now(), 'search_status' => 'request_failed', 'last_error' => 'Empty HTML response.']);
                    continue;
                }
                ...
                if (!$this->pageLooksRelevantForProduct($link, $matchedTitle, $url)) {
                    ...
                    $link->update(['last_checked_at' => now(), 'search_status' => 'mismatch', 'last_error' => 'Page title/url does not match product.', 'matched_title' => $matchedTitle, 'competitor_product_name' => $matchedTitle]);
                    Log::warning('Skipped mismatched product page', ['url' => $url, 'link_id' => $link->id, 'matched_title' => $matchedTitle, 'store' => $link->store?->name]);
                    continue;
                }
                ...
                if ($price !== null) {
                    ...
                    $priceChanged = $oldPrice === null || round((float)$oldPrice, 2) !== round((float)$price, 2);
                    ...
                    if ($priceChanged) {
                        ...
                        if ($ourPrice !== null) {
                            ...
                            if ((float)$price > 0) $percentDifference = round((((float)$ourPrice - (float)$price) / (float)$price) * 100, 2);
                            if ((float)$ourPrice < (float)$price)     { $position = '#1 Best Price'; $status = 'Cheaper'; }
                            elseif ((float)$ourPrice > (float)$price) { $position = 'Not Best Price'; $status = 'More Expensive'; }
                            else                                       { $position = 'Same Price'; $status = 'Match'; }
                        }
                    ...
                    Log::info('Price updated', ['url' => $url, 'price' => $price, 'link_id' => $link->id]);
                } else {
                    $this->warn('Price not found.');
                    $link->update(['last_checked_at' => now(), 'search_status' => 'price_not_found', 'last_error' => 'Could not extract price from HTML.', 'matched_title' => $matchedTitle, 'competitor_product_name' => $matchedTitle]);
                    Log::warning('Price not found', ['url' => $url, 'link_id' => $link->id]);
                }
            } catch (\Throwable $e) {
                $this->error('Error: ' . $e->getMessage());
                $link->update(['last_checked_at' => now(), 'search_status' => 'error', 'last_error' => $e->getMessage()]);
                Log::error('Price check error', ['url' => $url, 'link_id' => $link->id, 'message' => $e->getMessage()]);
            }
        }
    ...
    protected function updateOwnProductPrices(?int $productId, OwnProductPriceService $ownPriceService): void
    {
        ...
        foreach ($products as $product) {
            ...
            if ($productUrl === '' || $productUrl === '#') continue;

            try {
                $newOwnPrice = $ownPriceService->getPrice($productUrl);
                if ($newOwnPrice === null || (float)$newOwnPrice <= 0) continue;
                ...
                if ($oldOwnPrice === null || $oldOwnPrice !== $newOwnPrice) {
                    ...
                    $product->save();
                    Log::info('Own price updated', ['product_id' => $product->id, 'old_price' => $oldOwnPrice, 'new_price' => $newOwnPrice]);
                    $this->createOwnPriceNotification($product, $oldOwnPrice, $newOwnPrice);
                }
            } catch (\Throwable $e) {
                Log::error('Own price update error', ['product_id' => $product->id, 'url' => $productUrl, 'message' => $e->getMessage()]);
            }
        }
    }
    ...
    protected function createOwnPriceNotification(Product $product, ?float $oldPrice, ?float $newPrice): void
    {
        ...
        $exists = Notification::query()->where('product_id', $product->id)->where('type', 'own_price_changed')->where('message', $message)->where('created_at', '>=', now()->subMinutes(10))->exists();
        if ($exists) return;

        Notification::create(['product_id' => $product->id, 'type' => 'own_price_changed', 'message' => $message, 'is_read' => false]);
    }
    ...
    protected function createPriceNotification(CompetitorLink $link, $oldPrice, $newPrice, string $storeName): void
    {
        ...
        $exists = Notification::query()->where('product_id', $link->product_id)->where('type', 'price_changed')->where('message', $message)->where('created_at', '>=', now()->subMinutes(10))->exists();
        if ($exists) { Log::info('Duplicate notification skipped', ['product_id' => $link->product_id]); return; }

        Notification::create(['product_id' => $link->product_id, 'type' => 'price_changed', 'message' => $message, 'is_read' => false]);
    }

    protected function formatPriceText(?float $price): string
    {
        if ($price === null) return 'няма цена';
    ...
    {
        if (str_contains($url, 'zora.bg')) {
            return $this->fetchZoraPageShell($url);
        }

        $response = Http::timeout(15)->connectTimeout(6)->retry(2, 700)->withHeaders([
    ...
    protected function fetchZoraPageShell(string $url): array
    {
        ...
        Log::info('Zora shell curl result', ['url' => $url, 'status' => $status, 'body_len' => strlen($body)]);
    ...
    protected function pageLooksRelevantForProduct($link, ?string $matchedTitle, string $url): bool
    {
        ...
        if (strlen($ean) >= 8) {
            ...
            if ($eanMatch) return true;
            $checks[] = false;
        }

        $brand = trim((string)($product->brand ?? ''));
        if ($brand !== '' && $brand !== '-' && mb_strlen($brand) >= 3) {
            $brandMatch = str_contains($haystackRaw, mb_strtolower($brand)) || str_contains($haystackAscii, mb_strtolower(Str::ascii($brand)));
        ...
        if ($name !== '') {
            ...
            foreach (preg_split('/\s+/u', mb_strtolower($name)) as $word) {
                ...
                if (str_contains($haystackRaw, $word) || str_contains($haystackAscii, mb_strtolower(Str::ascii($word)))) $nameHits++;
            }
        ...
        if (!$result) {
            Log::debug('Page relevance: rejected', ['product_id' => $product->id, 'url' => $url, 'matched_title' => $matchedTitle, 'checks' => $debugInfo]);
        }
