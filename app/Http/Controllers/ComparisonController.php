...
class ComparisonController extends Controller
{
    ...
    private function buildProcessedProductsCollection(
        ...
        $products = $productsQuery->latest()->get()->map(function ($product) {
            ...
            $pazaruvajLowest = $lowestOffer
                ? (float) $lowestOffer->price
                : $this->normalizePrice($pazaruvaj?->last_price);
    ...
    public function tvDashboard()
    {
        ...
        $top = $products->filter(function ($product) {
            if (($product->pazaruvaj_offers_count ?? 0) < 2) {
                return false;
            }

            if ((int) ($product->pazaruvaj_our_position ?? 0) !== 1) {
                return false;
            }
            ...
            $nextCompetitor = $sortedOffers->first(function ($offer) use ($ourPrice) {
                return (float) $offer->price > $ourPrice;
            });
            ...
            $leadEuro = (float) $nextCompetitor->price - $ourPrice;
    ...
    public function tvDashboardData(Request $request)
    {
        ...
        $notTopAll = $products->filter(function ($product) {
            ...
            $positionWeight = (int) ($product->pazaruvaj_our_position ?? 0) * 1000;
            $diffWeight = abs((float) ($product->difference_amount ?? 0)) * 100;
            $offersWeight = (int) ($product->pazaruvaj_offers_count ?? 0) * 10;

            return $positionWeight + $diffWeight + $offersWeight;
        })->values();

        $topAll = $products->filter(function ($product) {
            $offers = collect($product->pazaruvaj_offers_list ?? [])->values();
