...
<div class="container py-4" style="max-width:900px;">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap;margin-bottom:20px;">
        <div>
            <h1 style="margin:0;font-size:28px;font-weight:700;">Edit User</h1>
            <p style="margin:6px 0 0;color:#6b7280;">Update account details, role and status.</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}">
            ...
            <div style="display:grid;grid-template-columns:repeat(2, minmax(0,1fr));gap:16px;">
                ...
                <div>
                    ...
                    <input type="password" name="password"
                           style="width:100%;padding:12px 14px;border:1px solid #d1d5db;border-radius:12px;">
                    <small style="display:block;margin-top:6px;color:#6b7280;">Leave empty to keep current password.</small>
                </div>
                ...
                <div>
                    ...
                    <select name="role"
                        ...
                        @foreach($roles as $value => $label)
                            ...
                            {{-- ADMIN НЕ ВИЖДА SUPER ADMIN --}}
                            @if(auth()->user()->isAdmin() && $value === \App\Models\User::ROLE_SUPER_ADMIN)
                                @continue
                            @endif

                            {{-- НЕ ПОЗВОЛЯВАМЕ ДА СЕ ПИПА SUPER ADMIN --}}
            ...
            <div style="margin-top:22px;display:flex;gap:12px;flex-wrap:wrap;">
                ...
                <a href="{{ route('admin.users.show', $user) }}"
                   style="padding:12px 18px;border-radius:12px;background:#eef2ff;color:#3730a3;text-decoration:none;font-weight:700;">
                    View
                </a>
